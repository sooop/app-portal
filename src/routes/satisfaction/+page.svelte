<script>
    ...
    import ResultTableSatisfaction from "./ResultTableSatisfaction.svelte";
	...
	async function handleUpload(detail) {
		...
		try {
			const analysisResults = await analyzeSatisfactionData(
				rawData,
				/** @type {any} */ (window).XLSX,
			);
...
<div class="max-w-6xl mx-auto p-6 min-h-screen">
	<div class="bg-white rounded-xl shadow-lg p-8">
		...
		{#if error}
			<div
				class="bg-red-50 border border-red-200 rounded-lg p-4 my-4 text-center"
			>
		...
		<!-- 평균 만족도 표 -->
			{#if results.satisfactionAverages && Object.keys(results.satisfactionAverages).length > 0}
			<ResultTableSatisfaction {results} />
			{/if}
		{/if}
	</div>
</div>
