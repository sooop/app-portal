...
>
        <div
            ...
            class="bg-white rounded-lg shadow-xl w-full max-w-2xl h-full max-h-[90vh] flex flex-col"
        >
          <!-- 모달 헤더 - 고정 -->
          <div class="flex items-center justify-between p-6 border-b border-gray-200 flex-shrink-0">
            <h3 class="text-xl font-medium text-gray-900">강좌별 이수 조건 설정</h3>
            <button
              ...
              {#each Object.entries(subjectCompletionRates) as [subject, rate]}
                <div class="flex items-center justify-between p-4 border border-gray-200 rounded">
                  <div class="flex-1 pr-4">
                    <h4 class="font-medium text-gray-900 break-words">{subject}</h4>
                  </div>
                  <div class="flex-shrink-0">
                                          <select
                        value={Math.round(rate * 100)}
                        onchange={(e) => handleCompletionRateChange(subject, parseInt(/** @type {HTMLSelectElement} */ (e.target).value) / 100)}
                        class="px-3 py-2 border border-gray-300 rounded bg-white min-w-[80px]"
                      >
